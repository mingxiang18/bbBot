...
本轮 planner 后处理边界：AI 返回具体非 `GUIDE` 类型时以 AI 分类为主；只有命中“特别订单 vs 传说鱼/鱼塘”“鱼塘产物 vs 鱼塘建筑”“农场地图 vs 农场建筑”“通用攻略 vs 技能/制作/节日/农场地图/特别订单/鱼塘/怪物掉落”等高风险边界，才使用本地高置信规则纠偏；不会把 `UNKNOWN` 或明确 typed miss 强行改写成旧自由文本路由，避免问 A 答 B。
