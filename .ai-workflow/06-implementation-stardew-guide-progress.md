...
当前机制判断：方向合理，职责基本清晰。AI 负责理解用户意图和生成检索计划，本地代码负责少量高置信纠偏、确定性证据检索和安全兜底，最终回答再交给 AI 做表达整合。后续优化不应继续扩大旧自由文本路由，也不应让 retriever 自己猜类型；如果新增地点、宠物/马/史莱姆、居民复杂规则等类目，应先扩 typed intent 和结构化证据，再补 planner 分类规则和防串路测试。

本轮 planner 后处理边界：AI 返回具体非 `GUIDE` 类型时以 AI 分类为主；只有命中“普通任务 vs 特别订单/资源/居民”“特别订单 vs 传说鱼/鱼塘”“鱼塘产物 vs 鱼塘建筑”“农场地图 vs 农场建筑”“姜岛探索 vs 通用攻略/资源/商店/建筑/农场地图”“地下城 vs 通用攻略/资源/技能/农场地图”“通用攻略 vs 技能/制作/节日/农场地图/农场动物/普通任务/特别订单/鱼塘/怪物掉落/姜岛探索/地下城”等高风险边界，才使用本地高置信规则纠偏；AI 明确返回 `UNKNOWN` 时只允许被拉回到普通任务 `QUEST`，避免把“鸡舍升级材料”这类问题从未知硬拉到旧命中链导致问 A 答 B。Planner 不可用、空返回或 JSON 不可解析时，才使用完整本地 fallback plan。
...
工具类目已从“升级材料”扩展为完整 `TOOL` typed evidence。`tools` 现在覆盖 11 类工具：斧头、镐子、喷壶、锄头、垃圾桶、鱼竿、铜盘/钢盘/金盘/铱盘、镰刀/金镰刀/铱金镰刀、背包、奶桶、剪刀。工具本身的升级、购买、获取、用途、费用和前置条件走 `TOOL`，例如“钢斧需要什么材料”“铱盘升级需要什么”“铱金镰刀怎么拿”“背包升级多少钱”“奶桶在哪里买”；工具附魔、武器锻造、精通系统怎么规划仍走 `GUIDE`。

姜岛探索类目已从 `GUIDE` / `RESOURCE` / `SHOP` / `BUILDING` 边界中拆出 `ISLAND` typed intent 和 `islandGuides` 专用数据。姜岛整体解锁、威利修船、南/东/北/西区域、海滩度假村、海盗湾、美人鱼、宝石鸟、挖掘场、蜗牛教授/田野办事处、岛屿商人、岛屿农场/美食家青蛙、伯蒂/沉船/打地鼠、齐先生核桃房、鹦鹉快线等走 `ISLAND`，例如“姜岛怎么解锁”“岛屿农场怎么修”“海盗湾怎么进”“美人鱼谜题怎么做”“蜗牛教授怎么救”；具体物品来源、火山地牢打法、黄貂鱼钓点、齐先生特别订单仍分别走 `RESOURCE`、`DUNGEON`、`FISH`、`SPECIAL_ORDER`。
...
已结构化覆盖鱼类、收集包、居民、作物、建筑、完整工具获取/升级、完整制作配方、机器兼容、商店/兑换、资源、怪物、鱼塘、料理/书籍、普通任务/任务日志、特别订单/齐先生任务、技能、节日/活动、农场地图、农场动物、姜岛探索、地下城/冒险地点、通用攻略等高频类目。

当前数据量级：鱼类 74 条，收集包 58 个，居民 34 位，作物 30 个，建筑 27 个，工具 11 类，完整制作配方 150 个，机器兼容数据 80 个，商店/兑换 29 个，资源 181 项，怪物 58 个，鱼塘 73 条，料理/饮品 83 道，书籍 26 本，普通任务 55 个，特别订单 28 个，技能攻略 9 个，节日/活动 12 个，农场地图 8 个，农场动物 11 个，姜岛探索 13 个，地下城/冒险地点 6 个，通用攻略 39 项。

居民已完成可送礼居民的首轮可发布覆盖：生日、礼物偏好、普通日程和高频雨天/星期/诊所/上课规则。更复杂的姜岛度假、婚后、好感剧情、绿雨、沙漠节期间特殊行为等放入后续增强。

## 验证结论

最近一次星露谷聚焦单元测试通过：`261 tests, 0 failures`。

最近一次 `bb-bot-server` reactor compile 通过：`BUILD SUCCESS`。

JSON 轻量校验通过：`tools=11`，`islandGuides=13`，`dungeonGuides=6`，`craftingRecipes=150`，`storyQuests=55`，`specialOrders=28`，`festivalEvents=12`，`farmMaps=8`，`farmAnimals=11`，`machines=80`，`resources=181`，`guides=39`，`skillGuides=9`；重复 id 为空。`git diff --check` 对本轮相关文件通过。
...
后续优先补齐农场布局摆放细节、居民复杂规则、宠物/马/史莱姆等偏门动物数据，以及剩余地点/系统类攻略。每轮补数都要同步补 typed intent、防串路单元测试、JSON 校验、聚焦单元测试、reactor compile 和 `git diff --check`。
