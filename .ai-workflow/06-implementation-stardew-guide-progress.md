...
- 资源获取：169 项，覆盖硬木、电池组、铱矿石、五彩碎片、上古种子、布料、三种钓鱼果冻，煤炭、黏土、纤维、苔藓、精炼石英、三类树液采集物、干草、海草/绿藻、树液、铜铁金矿石等高频材料，以及太阳精华、虚空精华、蝙蝠翅膀、史莱姆泥、虫肉、骨头碎片等高频怪物掉落，恐龙蛋、恐龙蛋黄酱、矮人卷轴、兔子的脚、鱼籽酱、龙牙、远古斑点、古物宝藏、四类晶球、全 53 件矿物、全 42 件古物、动物产品、果树水果，以及齐钻、金核桃、齐币、星星币、奖券、三花蛋、火山晶石、金色标签 8 个特殊货币/兑换物
- 怪物掉落表：58 个怪物/怪物变种条目，覆盖 Wiki `Category:Monsters` 当前分类页；支持“煤尘精灵掉什么”“飞蛇在哪刷”“熔岩潜伏怪掉落”等怪物 -> 掉落/地点/经验问题，并和“虚空精华哪里刷”等资源 -> 获取方式问题分开检索
- 鱼塘产物/扩容任务：73 条，覆盖 Fish Pond 产物表中的全部可养鱼/蟹笼/珊瑚/海胆条目，含初始容量、上限、自然增殖、扩容任务、产物概率、每日概率和推荐用途；支持“鲟鱼鱼塘产什么”“岩浆鳗鱼鱼塘要什么”“鱼塘养什么好”等问题，并和“鱼塘建造材料多少钱”分开路由
...
- 通用攻略：38 项，新增特殊货币与兑换物总览，并加厚精通系统、火山锻造与附魔；覆盖特殊货币规划、武器宝石锻造、工具/武器附魔、无限武器、银河之魂、戒指合成、精通点、五系精通奖励和精通优先级
...
- 特殊货币/兑换物：特殊货币有哪些、齐钻怎么获得、金核桃怎么用、齐币在哪里买、星星币怎么刷、三花蛋换什么、火山晶石怎么用、金色标签怎么获得
- 本地库没有建模的问题走 Wiki 兜底，如秘密纸条路线等细节解谜
...
2026-06-26 全矿物资源获取补齐：

- 按“完整一类数据”要求补齐博物馆 53 个矿物资源，不再只补高频矿物。
- 全矿物集合覆盖：
  - 4 个可采集矿物：石英、地晶、泪晶/冰泪晶、火水晶。
  - 8 个宝石：绿宝石、海蓝宝石、红宝石、紫水晶、黄水晶、翡翠、钻石、五彩碎片。
  - 41 个晶球矿物：虎眼石、猫眼石、赤红猫眼石、透闪石、黑方石、重晶石、青泥石、方解石、白云石、硅钙石、氟磷灰石、杰明石、日光榴石、绿水镍矿、铁铅矿、蓝晶石、酸性月岩、孔雀石、柱星叶石、柠檬石、新硅钙石、雌黄、石化史莱姆、雷公蛋、黄铁矿、海洋石、幽灵水晶、碧玉、天青石、大理石、沙岩、花岗岩、玄武岩、石灰石、皂石、赤铁矿、泥石、黑曜石、板岩、精灵石、陶瓷碎片。
- 每个矿物条目包含获取方式、对应晶球/矿点/怪物补充来源、博物馆捐赠用途和补缺建议；特殊用途补充了楼梯兑换、香辣鳗鱼、鱼塘任务、大理石火炬、黑曜石/虎眼石送礼等高频信息。
- 检索框架调整：
  - `StardewQueryPlannerService` prompt 明确“具体矿物怎么获得/开哪个晶球”归 `RESOURCE`，“博物馆整体补全/缺矿物路线”归 `MUSEUM`。
  - fallback 新增 `looksLikeSpecificMineralResourceQuery`，黄水晶、大理石、陶瓷碎片、黑曜石等具体矿物查询不再因为包含“矿物”而误归博物馆总览。
  - `StardewGuideTool` 描述补全“全 53 矿物、具体矿物怎么获得/开哪个晶球”。
- 测试补充：
  - `StardewKnowledgeRepositoryTest` 强制校验 53 个矿物资源 id 全存在，每个都有获取方式、博物馆捐赠用途和来源。
  - `StardewGuideServiceTest` 覆盖旧 `answer(String)` 与 typed `RESOURCE` 路径下的黄水晶、绿宝石、大理石、黑方石、陶瓷碎片、虎眼石。
  - `StardewGuideRetrieverTest` 覆盖 typed `RESOURCE` 多矿物检索，确认不串到博物馆 95 件/42 古物/53 矿物总览。
  - `StardewQueryPlannerServiceTest` 覆盖 AI 不可用时具体矿物归 `RESOURCE`、博物馆缺矿物归 `MUSEUM`。

本轮验证结果：

- JSON 轻量校验：`resources=134`，`minerals=53`，`missing=[]`，`dup=[]`，`withoutMuseumUse=[]`。
- `git diff --check`：无 whitespace 问题。
- `StardewKnowledgeRepositoryTest,StardewGuideServiceTest,StardewGuideRetrieverTest,StardewQueryPlannerServiceTest`：162 tests, 0 failures。
...
2026-06-26 完整怪物掉落表与 `MONSTER_DROP` intent：

- 数据补齐：
  - 新增 `monsterDrops` 结构化数据段，写入 58 个 Wiki `Category:Monsters` 当前分类页对应的怪物/危险变种/聚合条目。
  - 代表覆盖：煤尘精灵、飞蛇、皇家飞蛇、熔岩潜伏怪、霸王喷火龙、蓝鱿鱼、暗影狂徒、骷髅、蝙蝠、史莱姆、荒野石魔等。
  - 每条包含中文名、英文名、别名、出现地点、楼层/条件、战斗经验、掉落列表、刷法建议和内部来源 URL。
- 检索框架调整：
  - 新增 `StardewGuideIntent.MONSTER_DROP`，把“某个怪物掉什么/在哪刷/楼层/战斗经验”从 `RESOURCE` 拆出来。
  - `RESOURCE` 继续负责“某个物品怎么获得/哪里刷”，例如“虚空精华哪里刷”“蝙蝠翅膀怎么获得”。
  - `StardewQueryPlannerService` 的 AI prompt 和本地 fallback 都已加入 `MONSTER_DROP` 分类规则，避免 AI 不可用时回到旧的模糊命中。
  - `StardewGuideRetriever` typed retrieval 已支持 `MONSTER_DROP` hint，`StardewGuideService` 新增 typed 怪物掉落答案。
- 测试补充：
  - `StardewKnowledgeRepositoryTest`：硬性校验 `monsterDrops=58`，并覆盖关键怪物条目和所有条目核心字段。
  - `StardewGuideServiceTest`：覆盖煤尘精灵/飞蛇/熔岩潜伏怪掉落表，不串到资源获取或怪物图鉴书籍。
  - `StardewGuideRetrieverTest`：覆盖混合问题“怪物掉落 + 资源获取”不会互相抢路由。
  - `StardewQueryPlannerServiceTest`：覆盖 AI 失败时 fallback 将怪物掉落问法归入 `MONSTER_DROP`，物品获取仍归入 `RESOURCE`。

本轮验证结果：

- `StardewQueryPlannerServiceTest,StardewGuideRetrieverTest,StardewGuideAssistantServiceTest,StardewGuideServiceTest,StardewGuideToolTest,BbStardewHandlerTest,StardewWikiApiClientTest,StardewKnowledgeRepositoryTest`：183 tests, 0 failures。
- `-pl bb-bot-server -am -DskipTests compile`：BUILD SUCCESS。

2026-06-26 完整鱼塘产物与 `FISH_POND` intent：

- 数据补齐：
  - 新增 `fishPonds` 结构化数据段，写入 Fish Pond 产物表中的 73 个条目。
  - 每条包含鱼名/英文名/别名、初始容量、鱼塘上限、自然增殖、扩容任务、产物、产物池概率、总体每日概率、推荐用途和内部来源 URL。
  - 代表覆盖：鲟鱼、岩浆鳗鱼、水滴鱼、冰柱鱼、黄貂鱼、午夜鱿鱼、史莱姆鱼、虚空鲑鱼、珊瑚、海胆、鱼王和齐先生大家族任务传说鱼 II。
- 检索框架调整：
  - 新增 `StardewGuideIntent.FISH_POND`，把“某鱼鱼塘产什么/扩容要什么/鱼塘养什么好/鱼籽酱”从普通鱼类、建筑和资源里拆出来。
  - `StardewQueryPlannerService` 的 AI prompt 和本地 fallback 都已加入 `FISH_POND` 分类规则。
  - `鱼塘建造材料多少钱` 仍归 `BUILDING`，不会被鱼塘产物抢走；普通鱼详情和料理，如 `dorado 怎么钓`、`香辣鳗鱼材料和效果`，也不会因为鱼名存在鱼塘数据而跑偏。
  - `StardewGuideTool` 描述继续保持短描述，只负责工具选择，不把鱼塘等内部类别塞回 tool 注解。
- 测试补充：
  - `StardewKnowledgeRepositoryTest`：强制校验 `fishPonds=73`，覆盖鲟鱼、岩浆鳗鱼、黄貂鱼、珊瑚、海胆、鱼王等关键条目和所有条目核心字段。
  - `StardewGuideServiceTest`：覆盖鲟鱼鱼塘产物、岩浆鳗鱼扩容任务、鱼塘推荐列表，以及鱼塘建筑问题不串路。
  - `StardewGuideRetrieverTest`：覆盖 typed `FISH_POND` + `BUILDING` 混合检索，不串到普通鱼类或工具升级。
  - `StardewQueryPlannerServiceTest`：覆盖 AI 不可用时鱼塘产物/推荐归 `FISH_POND`，鱼塘建造归 `BUILDING`，夏季钓鱼仍归 `FISH`。

本轮验证结果：

- `StardewQueryPlannerServiceTest,StardewGuideRetrieverTest,StardewGuideAssistantServiceTest,StardewGuideServiceTest,StardewGuideToolTest,BbStardewHandlerTest,StardewWikiApiClientTest,StardewKnowledgeRepositoryTest`：188 tests, 0 failures。
- `-pl bb-bot-server -am -DskipTests compile`：BUILD SUCCESS。

2026-06-26 特殊货币/兑换物结构化补齐：

- 数据补齐：
  - 新增 8 个特殊货币/兑换物资源条目：齐钻、金核桃、齐币、星星币、奖券、三花蛋、火山晶石、金色标签。
  - 每条包含中文/英文别名、获取方式、使用建议、常见用途和来源 URL。
  - 新增 `special_currencies` 通用攻略，按永久进度类、赌场和节日类、票券类组织总览和规划建议。
- 检索框架调整：
  - `StardewGuideTool` 的 `@AiTool.description` 进一步收敛为短入口描述，只负责让星露谷攻略问题进入工具，不再承载内部分类清单。
  - `StardewQueryPlannerService` prompt 和 fallback 明确：泛问“特殊货币/各种币怎么花”归 `GUIDE`；具体问“齐钻怎么获得/金核桃怎么用/三花蛋换什么/金色标签怎么获得/火山晶石怎么用”归 `RESOURCE`。
  - `StardewGuideService` 兼容旧入口时优先保护具体特殊货币资源问法，避免 `火山晶石怎么用` 被火山锻造总览抢答。
- 测试补充：
  - `StardewKnowledgeRepositoryTest` 校验 8 个特殊货币资源和 `special_currencies` 总览都存在，并且核心字段/来源齐全。
  - `StardewGuideServiceTest` 覆盖齐钻、金核桃、齐币、星星币、三花蛋、火山晶石、金色标签和特殊货币总览，确认不串到陶瓷碎片、星之碎片、工具升级或锻造总览。
  - `StardewGuideRetrieverTest` 覆盖 typed `RESOURCE + GUIDE` 混合检索，确认特殊货币不会跑到商店、古物/矿物或火山锻造总览。
  - `StardewQueryPlannerServiceTest` 覆盖 AI 不可用时特殊货币总览归 `GUIDE`，具体货币问法归 `RESOURCE`。

本轮验证结果：

- `StardewQueryPlannerServiceTest,StardewGuideRetrieverTest,StardewGuideAssistantServiceTest,StardewGuideServiceTest,StardewGuideToolTest,BbStardewHandlerTest,StardewWikiApiClientTest,StardewKnowledgeRepositoryTest`：193 tests, 0 failures。
- JSON 轻量校验：`resources=169`，`guides=38`，8 个特殊货币资源 `missing=[]`，`weak=[]`，重复 resource id 为空，`special_currencies` 4 个章节齐全。
...
- 资源获取已扩到 169 项，新增一批博物馆、全 42 件古物、全 53 件矿物、晶球、稀有物品、动物产品、果树水果、特殊货币/兑换物和首批高频怪物掉落；怪物掉落表已单独拆成 `monsterDrops=58` 结构化类目；鱼塘产物/扩容任务已单独拆成 `fishPonds=73` 结构化类目；但全姜岛/火山材料的非货币部分还未完整结构化。
...
1. 数据覆盖：
   ...
   - 矿井、骷髅洞穴、火山、怪物掉落、博物馆、烹饪、制作；首批 169 个资源获取条目已结构化，已补煤炭、太阳精华、虚空精华、蝙蝠翅膀、史莱姆泥、虫肉、骨头碎片等高频怪物掉落，并已补全 53 件矿物、42 件古物、58 个怪物掉落表条目、73 个鱼塘产物/扩容任务条目和 8 个特殊货币/兑换物条目，后续继续补姜岛/火山非货币材料
2. 测试覆盖：
