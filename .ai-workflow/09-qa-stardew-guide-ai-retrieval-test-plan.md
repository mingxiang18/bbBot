...
- 分类职责边界：AI 返回具体非 `GUIDE` 类型时以 AI 分类为主；本地规则只纠偏高风险串路，例如通用 `GUIDE` 被拉回明确的 `CRAFTING`、`FESTIVAL` 或 `FARM_MAP`，以及农场地图与农场建筑互相误判。
...
- 本地数据加载日志规模符合预期：fish=74、bundles=58、crops=30、buildings=27、tools=6、craftingRecipes=150、machines=80、shops=29、villagers=34、resources=181、monsterDrops=58、fishPonds=73、cookingRecipes=83、books=26、specialOrders=28、skillGuides=9、festivalEvents=12、farmMaps=8、guides=39。
...
当前结果：232 tests, 0 failures, 0 errors, 0 skipped。
