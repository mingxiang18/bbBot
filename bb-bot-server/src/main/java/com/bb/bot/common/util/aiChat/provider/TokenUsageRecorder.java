...
public class TokenUsageRecorder {
     ...
     * @param prompt       prompt token 数（含 cache-hit）
     * @param cached       命中缓存（cache read）的输入 token 数（按 cache-hit 单价计）
     * @param cacheWrite   写缓存（cache creation）的输入 token 数（按 cache-write 单价计，仅 Anthropic）
     * @param completion   completion token 数（含 reasoning）
     * @param total        total token 数（部分网关只给 total）
     */
    public void record(String providerName, String model, int prompt, int cached, int cacheWrite, int completion, int total) {
        ...
        try {
            writePool.execute(() -> persist(userId, platform, sessionId, role,
                    providerName, model, prompt, cached, cacheWrite, completion, total));
        } catch (Exception e) {
    ...
    private void persist(String userId, String platform, String sessionId, String role,
                         ...
                         int prompt, int cached, int cacheWrite, int completion, int total) {
        try {
            BigDecimal cost = modelPricingService.costCny(providerName, model, prompt, cached, cacheWrite, completion);
            ...
            row.setCacheWriteTokens(cacheWrite);
            row.setCostCny(cost);
            row.setCreatedAt(LocalDateTime.now());
            tokenUsageService.save(row);
            // 全局每日 token 兜底计数
            globalUsageGuard.add(total);
        } catch (Exception e) {
