 ...
 * @author ren
 */
@BootEventHandler(botType = BotType.BB, name = "宝可梦(测试)")
public class PokemonHandler {
    ...
    public void getPokemonHandle(BbReceiveMessage bbReceiveMessage) {
        ...
        bbSendMessage.setMessageList(Arrays.asList(
                ...
                BbMessageContent.buildLocalImageMessageContent(new File(FileUtils.getAbsolutePath("pokemon/origin/" + pokemonDataList.get(index).getId() + ".png")))));
        bbMessageApi.sendMessage(bbSendMessage);
    }
    ...
    public void combinePokemonHandle(BbReceiveMessage bbReceiveMessage) {
        ...
        String beforeNameJson = new String(FileUtils.getFile("pokemon/before_name.json"), StandardCharsets.UTF_8);
        List<PokemonData> beforeNameList = JSON.parseObject(beforeNameJson, new TypeReference<List<PokemonData>>() {});

        String afterNameJson = new String(FileUtils.getFile("pokemon/after_name.json"), StandardCharsets.UTF_8);
        ...
        bbSendMessage.setMessageList(Arrays.asList(
                ...
                BbMessageContent.buildLocalImageMessageContent(new File(FileUtils.getAbsolutePath("pokemon/combine/" + afterNameList.get(id2).getId() + "-" + beforeNameList.get(id).getId() + ".png")))));
        bbMessageApi.sendMessage(bbSendMessage);
    }
}
